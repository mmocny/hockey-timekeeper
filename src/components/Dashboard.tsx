...
import { playersStore, nextShift, moveLane, resetGame } from '../lib/store';
import { CompactPlayer } from './CompactPlayer';
import { RotateCcw, ChevronRight } from 'lucide-react';

const LANE_NAMES = ['Center', 'Left Wing', 'Right Wing', 'Left Def', 'Right Def', 'Bench'];

const LaneRow: React.FC<{ laneIdx: number; playerList: any[]; onShiftClick: (idx: number) => void }> = ({ laneIdx, playerList, onShiftClick }) => {
  const lanePlayers = playerList
    .filter(p => p.lane === laneIdx)
    .sort((a, b) => a.queue_order - b.queue_order);
  
  const onIce = lanePlayers.find(p => p.is_on_ice);
  const others = lanePlayers.filter(p => !p.is_on_ice);
  const onDeck = others[0];
  ...
  return (
    <div className="py-2 border-b border-slate-900 last:border-0">
      ...
      <div className="flex items-center gap-2 min-h-[44px] px-1">
        {/* On Ice Slot */}
        <div className="w-28 shrink-0" onClick={() => onShiftClick(laneIdx)}>
          {onIce ? <CompactPlayer player={onIce} /> : <div className="w-28 h-11 rounded border border-dashed border-slate-800 flex items-center justify-center text-[9px] text-slate-700 font-bold uppercase">Empty</div>}
        </div>
        ...
        {/* On Deck Slot */}
        <div className="w-28 shrink-0" onClick={() => onShiftClick(laneIdx)}>
          {onDeck ? <CompactPlayer player={onDeck} /> : <div className="w-28 h-11 bg-slate-950/50 rounded border border-slate-900 border-dashed" />}
        </div>
  ...
  return (
    <div className="flex flex-col select-none">
      ...
      <div className="space-y-1">
        {[0, 1, 2, 3, 4].map(idx => (
          <LaneRow key={idx} laneIdx={idx} playerList={playerList} onShiftClick={nextShift} />
        ))}
      </div>
